...
int solve()
{
  ...
  SegmentTree<monoid_max> segmax(N);
  SegmentTree<monoid_min> segmin(N);
  for(int i = 0;i < N;i++){
    segmax.set(i,A[i]);
    segmin.set(i,A[i]);
  }
  ...
  ll ans = 0;
  for(int i = 0;i < N;i++){
    ll a = segmax.find_first(i,X);
    ll b = segmax.find_first(i,X+1);
    ll c = segmin.find_first(i,Y);
    ll d = segmin.find_first(i,Y-1);
    if (b == -1){
      b = N;
    }
    if (d == -1){
