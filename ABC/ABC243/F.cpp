...
int solve()
{
  ...
  Combination<modint> C(200);
  vector<vector<vector<modint>>> DP(N+1,vector<vector<modint>>(M+1,vector<modint>(K+1,0)));
  DP[0][0][0] = 1;
  for(int i = 0;i < N;i++){
    for(int j = 0;j <= M;j++){
      for(int k = 0;k <= K;k++){
        for(int l = 0;l + k <= K;l++){
          if (l == 0){
            ...
            modint p = W[i]/total_w;
            DP[i+1][j+1][k+l] += DP[i][j][k] * C.C(k+l,l) * p.pow(l);
          }
