...
int solve()
{
  ...
  for(int i = 1;i < N;i++){
    vector<modint> ruiseki(M+1,0);
    for(int j = 0;j < M;j++){
      ruiseki[j+1] = ruiseki[j] + DP[i-1][j];
    }
    for(int j = 0;j < M;j++){
      ll remove_from =  max(j - K + 1,0ll);
      ll remove_to = min(j + K,M);
      if (K == 0){
        DP[i][j] = ruiseki[M];
      }else{
        DP[i][j] = ruiseki[M] - (ruiseki[remove_to] - ruiseki[remove_from]);
      }
    }
  }
  modint ans = 0;
