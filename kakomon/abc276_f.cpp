...
int solve()
{
    ...
    SegmentTree<monoid_sum> seg_sum(200001);
    SegmentTree<monoid_sum> seg_count(200001);
    modint total = 0;
    for(int i = 0;i < N;i++){
        ll below_count = seg_count.query(0,A[i]);
        ll above_sum = seg_sum.query(A[i],200001);
        total += (below_count*2+1)*A[i];
        total += above_sum*2;
        seg_count.update(A[i],seg_count[A[i]]+1);
        seg_sum.update(A[i],seg_sum[A[i]]+A[i]);
        cout << total/(i+1)/(i+1) << "\n";
    }
    cout << flush;
    return 0;
