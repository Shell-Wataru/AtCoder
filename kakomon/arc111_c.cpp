...
int main()
{
    ...
    for(int i = 0;i < N;i++){
        auto min_wegiht_p = seg.query(0,N);
        ll weight = min_wegiht_p.first;
        ll index =  min_wegiht_p.second;
        if (p[index] == index){
            seg.update(index,{numeric_limits<ll>::max(),index});
            continue;
        }else if (weight <= b[p[index]]){
            cout << -1 << endl;
            return 0;
        }else{
            ll j = inv_p[index];
            changes.push_back({index+1,j+1});
            p[j] = p[index];
            inv_p[p[index]] = j;
            p[index] = index;
            inv_p[index] = index;
