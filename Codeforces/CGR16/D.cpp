...
int solve()
{
  ...
  vector<ll> b(a);
  sort(b.begin(),b.end());
  vector<map<ll,pair<ll,ll>>> ranges(n);
  for(ll i = 0;i < n;i++){
    for(ll j = 0;j < m;j++){
      if (ranges[i].find(b[m*i+j]) == ranges[i].end()){
        ranges[i][b[m*i+j]] = {numeric_limits<ll>::max(),numeric_limits<ll>::min()};
      }
      ranges[i][b[m*i+j]].first = min(ranges[i][b[m*i+j]].first,j);
      ranges[i][b[m*i+j]].second = max(ranges[i][b[m*i+j]].second,j);
    }
  }
  vector<SegmentTree<monoid_sum>> segs(n,SegmentTree<monoid_sum>(m));
  ll ans = 0;
  for(int i = 0;i < n*m;i++){
    for(int j = 0;j < n;j++){
      if (ranges[j].find(a[i]) != ranges[j].end()){
        ans += segs[j].query(0,ranges[j][a[i]].second);
        segs[j].update(ranges[j][a[i]].second,1);
        ranges[j][a[i]].second--;
        if (ranges[j][a[i]].second < ranges[j][a[i]].first){
          ranges[j].erase(a[i]);
        }
        break;
      }
