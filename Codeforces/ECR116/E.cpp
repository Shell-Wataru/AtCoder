...
int solve()
{
    ...
    scanf("%lld",&n);
    scanf("%lld",&x);
    vector<vector<modint>> DP(n+1,vector<modint>(x+1,0));
    for(int i = 1;i <= x;i++){
      DP[0][i] = 1;
    }
    for(int i = 2;i <= n;i++){
      ...
      for(int j = 1;j <= x;j++){
        modint maximum_x = j;
        if (j <= i-1){
          DP[i][j] += maximum_x.pow(i);
        }else{
          // DP[i][j] += base.pow(i);
          for(int k = 0;k <= i;k++){
            // cout << "i:" << i << "j:" << j << "k:" << k << "+=" << DP[i-k][j-(i-1)]*C.C(i,k)*base.pow(k) << endl;
            DP[i][j] += DP[i-k][j-(i-1)]*C.C(i,k)*base.pow(k);
          }
        }
      }
    }
