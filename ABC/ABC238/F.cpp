...
int main()
{
  ll N,M;
  cin >> N >> M;
  vector<map<vector<ll>,modint>> DP(N+1);
  DP[0][{0,numeric_limits<ll>::max(),numeric_limits<ll>::max(),numeric_limits<ll>::max()}] = 1;
  for(int i = 0;i < N;i++){
    for(auto p:DP[i]){
      for(int j = 1;j <= M;j++){
        auto next_v = p.first;
  ...
  modint ans = 0;
  for(auto p:DP[N]){
    if (p.first[3] != numeric_limits<ll>::max()){
      ans += p.second;
    }
  }
  cout << ans << endl;
