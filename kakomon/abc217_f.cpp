...
ll solve()
{
    ll N, M;
    cin >> N >> M;
    vector<vector<bool>> is_pair(2*N, vector<bool>(2*N, false));
    vector<vector<modint>> DP1(2*N+1, vector<modint>(2*N+1, false));
    vector<vector<modint>> DP2(2*N+1, vector<modint>(2*N+1, false));
    for (int i = 0; i < M; i++)
    {
        ll a, b;
        cin >> a >> b;
    ...
    for (int i = 2; i <= 2*N; i += 2)
    {
        for (int j = 0; i + j <= 2*N; j++)
        {
            if (is_pair[j][j + i - 1])
            {
                DP1[j][j + i] += DP2[j + 1][j + i - 1];
                DP2[j][j + i] += DP2[j + 1][j + i - 1];
            }
            for(int k = 2;k < i;k+= 2){
                DP2[j][j + i] += DP1[j][j + k] * DP2[j+k][j + i] * C.C(i/2,k/2);
            }
        }
    }
    // for(int i = 0;i <= 2*N;i++){
