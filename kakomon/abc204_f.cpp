...
int solve()
{
  ll H, W;
  cin >> H >> W;
  Matrix<modint> M(1ll << H);
  Matrix<modint> LastM(1ll << H);
  for (int i = 0; i < 1ll << H; i++)
  {
    vector<ll> current(1ll << H, 0);
    vector<ll> next(1ll << H, 0);
    current[i] += 1;
    for (int j = 0; j < H; j++)
    {
      for (int k = 0; k < 1ll << H; k++)
      {
        if (k & 1ll << j)
        {
          next[k & ~(1ll << j)] += current[k];
        }
        else
        {
          if (j + 1 < H && !(k & 1ll << (j + 1)))
