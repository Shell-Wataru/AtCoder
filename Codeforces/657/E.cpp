...
int solve()
{
    ...
    for (size_t i = 0; i < n; i++)
    {
        ...
        for (size_t j = 0; j < cells; j++)
        {
            ...
            scanf("%lld", &r);
            l--;
            result.push_back({{l, r},l});
            seg.set(l, {seg[l].first + 1, l});
        }
    }
    ...
    while (changed)
    {
        ...
        {
            auto p = result[i];
            auto max_pair = seg.query(p.first.first, p.first.second);
            if (max_pair.second != p.second)
            {
                changed = true;
                seg.update(p.second, {seg[p.second].first - 1, p.second});
                seg.update(max_pair.second, {max_pair.first + 1, max_pair.second});
                result[i].second = max_pair.second;
            }
        }
    }
    ll ans = 0;
