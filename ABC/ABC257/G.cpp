...
ll solve()
{
  ...
  int i = 1, j = 0;
  while (i < ST.size()) {
    while (i+j < ST.size() && ST[j] == ST[i+j]) ++j;
    A[i] = j;
    if (j == 0) { ++i; continue;}
    int k = 1;
    while (i+k < ST.size() && k+A[k] < j) A[i+k] = A[k], ++k;
    i += k; j -= k;
  }
  ...
  // }
  // cout << endl;
  SegmentTree<monoid_max> seg(T.size());
  for(int i = 0;i < T.size();i++){
    seg.set(i,A[S.size()+i]+i);
  }
  seg.build();
  ll k = 0;
  ll target = T.size();
  while(target != 0){
    ll a = seg.find_first(max(0ll,target-(ll)S.size()),target);
    if (a == -1 || a >= target){
      cout << -1 << endl;
      return 0;
    }else{
