...
//   cout << endl;
  SegmentTree<monoid_max> B(n);
  for(int i = 0;i < n;i++){
      B.set(i,upper_bound(A.begin(),A.end(),AB[i].second) - A.begin() - 1);
    //   cout << B[i] << ",";
  }
//   cout << endl;
  B.build();
  ll operation_count = 0;
  for(int i = n-1;i>= 0;i--){
      if (B[i] >= i){
          continue;
      }else{
        ll clear_index = B.find_first(0,i);
        // cout << "i:" << i << "clear_index:" << clear_index << endl;
        if (clear_index == -1 || clear_index > i){
            cout << "No" << endl;
            return 0;
        }
        ll target = B[clear_index];
        B.update(clear_index,B[i]);
        B.update(i,target);
        operation_count++;
      }
  }
//   cout << "op:" << operation_count << endl;
  if (operation_count <= n - 2){
      cout << "Yes" << endl;
  }else{
      cout << "No" << endl;
  }
