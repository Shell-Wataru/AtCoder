...
int main()
{
  ...
  cin >> N;
  modint two = 2;
  Combination<modint> C(300000);
  vector<modint> two_powers(N+1);
  two_powers[0] = 2;
  for(int i = 0;i < N;i++){
    two_powers[i+1] = two_powers[i]*two_powers[i];
  }
  vector<modint> three_powers(N+1);
  modint three = 3;
  modint bad = three.pow(N) - two.pow(N) - two.pow(N-1)*N;
  for(int i = 3;i <= N;i++){
    if (N % 2 == 0){
      bad += C.C(N,i) * two.pow(N-i);
    }else{
      bad += C.C(N,i) * two.pow(N-i);
    }
  }
  // cout << three_powers[N] << endl;
  // cout << two_powers[N] << endl;
  cout << two_powers[N] - bad << endl;
  return 0;
}
