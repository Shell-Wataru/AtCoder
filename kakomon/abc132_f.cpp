...
int solve()
{
    ...
    // }
    // cout << endl;
    vector<modint> ans(base.size());
    ans[1] = 1;
    for (int i = 0; i < k; i++)
    {
        vector<modint> plus_count(base.size()+1);
        for (int i = 1; i < base.size(); i++)
        {
            plus_count[1] += ans[i];
            plus_count[base.size()+1-i] += -ans[i];
        }
        for(int i = 0;i < base.size();i++){
            plus_count[i+1] += plus_count[i];
        }
        for(int i = 1;i < base.size();i++){
            ans[i] = plus_count[i]*(base[i] - base[i-1]);
        }
        // for (auto a : ans)
        // {
        //     cout << a << ",";
