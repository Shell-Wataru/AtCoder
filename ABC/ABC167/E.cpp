...
vector<modint> solve(ll N, modint M,modint invM)
{
  if (N == 1)
  {
    return vector<modint>(1, M);
  }
  vector<modint> A = solve(N / 2, M, invM);
  ArbitraryModConvolution<modint> fft;
  vector<modint> a = fft.multiply(A,A);
  ...
  //   for (int j = 0; j < N / 2; j++)
  //   {
  //     ans[i + j + 1] += A[i] * A[j] * invM;
  //     ans[i + j] += A[i] * A[j] - (A[i] * A[j] * invM);
  //   }
  // }
  if (N % 2 == 1){
    for(int i = N-1;i >= 0;i--){
...
int main()
{
  ...
  modint invM = 1;
  invM /=  M;
  vector<modint> ans = solve(N,M,invM);
  modint total = 0;
  for(int i = 0;i <= K;i++){
    // cout << ans[i] << ",";
    total += ans[i];
