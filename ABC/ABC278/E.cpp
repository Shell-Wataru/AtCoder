...
int main()
{
  ...
  vector<vector<vector<ll>>> ruisekiWa(N,vector<vector<ll>>(H+1,vector<ll>(W+1,0)));
  for(int i = 0;i < H;i++){
    for(int j = 0;j < W;j++){
      ll a;
      cin >> a;
      ...
      total[a]++;
      A[i][j] = a;
      ruisekiWa[a][i+1][j+1] = 1;
    }
  }
  for(int i = 0;i < N;i++){
    for(int j = 1;j <= H;j++){
      for(int k = 1;k <= W;k++){
        ruisekiWa[i][j][k] += ruisekiWa[i][j-1][k] + ruisekiWa[i][j][k-1] -ruisekiWa[i][j-1][k-1];
      }
    }
  }
  ...
  for(int i = 0;i+h <= H ;i++){
    for(int j = 0;j+w <= W;j++){
      ll ans = 0;
      for(int k = 0;k < N;k++){
        if (total[k] - (ruisekiWa[k][i+h][j+w] - ruisekiWa[k][i][j+w] - ruisekiWa[k][i+h][j] + ruisekiWa[k][i][j]) > 0){
          ans++;
        }
      }
      if (j != 0){
