...
int solve()
{
  ...
  Combination<modint> C(300001);
  modint remain = C.fact(n);
  for(int i = 0;i < 200000;i++){
    remain *= C.rfact(a_counts.sum(i,i+1));
  }
  modint ans = 0;
  for(int i = 0;i < min(n,m);i++){
    ans += remain * C.inv(n-i) * a_counts.sum(b[i]);
    remain *=  C.inv(n-i) * a_counts.sum(b[i],b[i]+1);
    a_counts.add(b[i],-1);
  }
  if (n < m){
    ans += remain;
