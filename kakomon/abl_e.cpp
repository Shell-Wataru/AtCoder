...
using S = pair<modint, ll>;
using F = pair<modint, bool>;
modint ten = 10;
S op(S x, S y) { return {x.first * ten.pow(y.second) + y.first, x.second + y.second}; }
S e() { return {0, 0}; }
S mapping(F f, S x)
{
    if (f.second)
    {
        return {f.first * ((ten.pow(x.second)) -1)/(ten - 1), x.second};
    }
    else
    {
        return x;
