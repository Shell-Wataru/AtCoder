...
int main()
{
  ll N,K;
  cin >> N >> K;
  Combination<modint> combination(100);
  vector<map<ll,modint>> DP(N+1);
  DP[0][1] = 1;
  for(int i = 1;i <= N;i++){
    for(int j = 0;j < i;j++){
      for(auto &p:DP[j]){
        ll lcm = p.first * (i-j)/ gcd(p.first,i-j);
        DP[i][lcm] += p.second * combination.fact(i-j-1) * combination.C(i-1,i-j-1);
      }
    }
  }
  modint ans = 0;
