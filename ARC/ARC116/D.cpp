...
int main()
{
    ...
    ll N,M;
    cin >> N >> M;
    Combination<modint> combination(10000);
    vector<vector<modint>> amari(2,vector<modint>(2*N+1));
    amari[0][0] = 1;
    for(int i = 13;i >= 0;i--){
      ll current = i % 2;
      ll next = (i+1) % 2;
      for(int j = 0;j <= 2*N;j++){
        ...
        for(int k = 0;k <= nokori;k+=2){
          ll tuminokoshi = (nokori - k) * 2;
          if (tuminokoshi <= 2*N){
            // cout << i <<"," << nokori << "," << k << endl;
            amari[next][tuminokoshi] += amari[current][j] *combination.C(N,k);
          }
        }
      }
      // cout << (M & 1<<i) << ":";
      ...
      // }
      // cout << endl;
      fill(amari[current].begin(),amari[current].end(),0);
    }
