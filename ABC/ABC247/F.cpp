...
int solve()
{
    ...
    for(int i = 0;i < N;i++){
        cin >> B[i];
        B[i]--;
        convertedB[i] = revA[B[i]];
        // cout << convertedB[i] << ",";
    }
    // cout << endl;
    vector<modint> vvDP(N+1,0);
    vector<modint> ovDP(N+1,0);
    vector<modint> voDP(N+1,0);
    vector<modint> ooDP(N+1,0);
    vvDP[1] = 1;
    ooDP[1] = 1;
    for(int i = 1;i < N;i++){
        vvDP[i+1] = voDP[i];
        voDP[i+1] = voDP[i] + vvDP[i];
        ovDP[i+1] = ooDP[i];
        ooDP[i+1] = ooDP[i] + ovDP[i];
    }
    UnionFind uf(N);
    for(int i = 0;i < N;i++){
        uf.unite(i,convertedB[i]);
    }
    modint ans = 1;
    for(int i = 0;i < N;i++){
        if (uf.find(i) == i){
            // cout << uf.size(i) << endl;
            ans *= ooDP[uf.size(i)] + ovDP[uf.size(i)] + voDP[uf.size(i)];
        }
    }
    cout << ans << endl;
    return 0;
