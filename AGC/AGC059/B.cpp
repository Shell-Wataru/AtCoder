...
int main()
{
  ...
  priority_queue<pair<ll,ll>,vector<pair<ll,ll>>,greater<pair<ll,ll>>> add_q;
  priority_queue<pair<ll,ll>,vector<pair<ll,ll>>,greater<pair<ll,ll>>> remove_q;
  for(int i = 0;i < N;i++){
    pair<ll,ll> p = {i,i};
    for(int l = i-1;l >= 0 && P[l] < P[i];l--){
      p.first = l;
    }
    for(int r = i+1;r < N && P[r] < P[i];r++){
      p.second = r;
    }
    ranges[i] = p;
    add_q.push({p.first,i});
  }
  ...
  for(int i = 0;i < N;i++){
    ...
    // }
    // cout << endl;
    vector<modint> nextDP(N+1);
    while(!add_q.empty() && add_q.top().first <= i){
      auto p = add_q.top();
      add_q.pop();
      numbers.insert(p.second);
      remove_q.push({ranges[p.second].second+1,p.second});
    }
    while(!remove_q.empty() && remove_q.top().first <= i){
      auto p = remove_q.top();
      remove_q.pop();
      numbers.erase(p.second);
    }
    ll current = 0;
    modint total = 0;
    for(auto n:numbers){
