...
int main()
{
  ...
  queue<pair<ll,ll>> q;
  vector<modint> patterns(N,0);
  patterns[0] = 1;
  q.push({0,0});
  vector<ll> distance(N,numeric_limits<ll>::max()/2);
  while(!q.empty()){
    ll current = q.front().first;
    ll d = q.front().second;
    q.pop();
    if (distance[current] != numeric_limits<ll>::max()/2){
      continue;
    }
    distance[current] = d;
    for(auto from:G[current]){
      if (distance[from] + 1 == distance[current]){
        patterns[current] += patterns[from];
      }
    }
