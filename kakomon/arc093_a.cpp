...
int solve()
{
    ...
    for(int i = 0;i < N;i++){
        if (i == 0){
            from_front[i] =  abs(A[i]);
        }else{
            from_front[i] = from_front[i-1] + abs(A[i]-A[i-1]);
        }
    }
    for(int i = N-1;i >= 0;i--){
        if (i == N-1){
            ...
            from_end[i] = from_end[i+1] + abs(A[i]-A[i+1]);
        }
    }
    for(int i = 0;i < N;i++){
        if (i == 0){
            cout << from_end[i+1] + abs(A[i+1]) << endl;
        }else if (i == N-1){
            cout << from_front[i-1] + abs(A[i-1]) << endl;
        }else{
            cout << from_front[i-1] + from_end[i+1] + abs(A[i-1]- A[i+1]) << endl;
        }
    }
    return 0;
}
