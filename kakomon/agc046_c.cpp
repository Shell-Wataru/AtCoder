...
int main()
{
    ...
    // cout << "!!" << endl;
    ones.push_back(current_ones);
    auto DP = make_vec(ones.size()+1,min(total_ones,K)+1,min(total_ones,K)+1,modint(0));
    // 何個目,前借り個数，総移動回数
    DP[0][0][0] = 1;
    for(int i = 0;i < ones.size();i++){
        for(int j = 0;j <= min(total_ones,K);j++){
            for(int k = 0;k <= min(total_ones,K);k++){
              if (DP[i][j][k] == 0){
                continue;
              }
              // cout << i << "," << j << "," << k << endl;
              for(int l = max(ones[i]-j,0ll);l <= min(total_ones,K)+ones[i]-k;l++){
                // cout << "l:" << l << endl;
                // cout << j+(l -ones[i]) << endl;
                // cout << k+max(l -ones[i],0ll) << endl;
                DP[i+1][j+(l -ones[i])][k+max(l -ones[i],0ll)] += DP[i][j][k];
              }
            }
        }
    }
    // cout << "!!" << endl;
    modint ans = 0;
    for(int i = 0;i <= min(total_ones,K);i++){
      ans += DP[ones.size()][0][i];
    }
    cout << ans << endl;
    return 0;
}
