...
int solve()
{
  ...
  for (int i = 0; i < n; i++)
  {
    ...
    modint base = ans;
    ll current = a[i] + kuriagari;
    //   cout << amari << ",";
    // cout << base - amari << endl;
    // cout << (base - amari) * (a[i] - 9) + amari << endl;
    ans = base + (base - amari) * a[i];
    if (a[i] >= 9 || (a[i] == 8 && kuriagari == 2)){
      amari = (base - amari) * (a[i] - 9) + amari;
    }else{
      amari = 0;
    }
    kuriagari = current/10;
