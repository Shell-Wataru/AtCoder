...
int solve()
{
    ...
    for (ll i = 0; i < n; i++)
    {
        ll max_depth = min(d, n - 1 - i);
        ll min_depth = d - max_depth;
        if (max_depth < min_depth){
            break;
        }
        if (max_depth == d){
            ans += two.pow(i) * two.pow(d)*2;
            ans += two.pow(i) * two.pow(d-2)*(d-1)*2;
        }else{
            ans += two.pow(i) * two.pow(d-2)*(max_depth- min_depth + 1)*2;
        }
        // cout << ans << endl;
    }
    cout << ans << "\n";
