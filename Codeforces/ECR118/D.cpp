...
int solve()
{
  ...
  map<pair<ll,bool>,modint> dp;
  dp[{0,false}]= 1;
  for(int i = 0;i < n;i++){
    map<pair<ll,bool>,modint> additions;

    additions[{a[i]-1,true}] += dp[{a[i]-1,false}];
    additions[{a[i]-1,true}] += dp[{a[i]-1,true}];
    additions[{a[i]+1,false}] += dp[{a[i],false}];
    // additions[{a[i]+2,false}] += dp[{a[i],true}];
    additions[{a[i]+1,false}] += dp[{a[i]+1,false}];
    additions[{a[i]+1,true}] += dp[{a[i]+1,true}];
