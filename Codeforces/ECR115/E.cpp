...
int solve()
{
    ...
    scanf("%lld",&m);
    scanf("%lld",&q);
    vector<SegmentTree<monoid_max>> seg_rows(n,SegmentTree<monoid_max>(n+m-1));
    vector<SegmentTree<monoid_max>> seg_columns(n+m-1,SegmentTree<monoid_max>(n));
    for(int i = 0;i < n-1;i++){
      seg_rows[i].update(n-2-i,1);
    }
    for(int i = 0;i < n-1;i++){
      seg_rows[i].update(n+m-1-i,1);
    }
    for(int i = 0;i < n+m-1;i++){
      seg_columns[i].update(n-2-i,1);
    }
    for(int i = 0;i < n-1;i++){
      seg_columns[i].update(n-2-i,1);
    }
