...
int main()
{
    ll N, A,B,C,D;
    cin >> N >> A >> B >> C >> D;
    vector<vector<modint>> DP(B-A+2,vector<modint>(N+1,0));
    Combination<modint> combination(2001);
    DP[0][N] = 1;
    // cout << "!!" << endl;
    for(int i = 0;i < B-A+1;i++){
        ll g = A + i;
        // cout << i<< ":"<< g << endl;
        modint g_rfact = combination.rfact(g);
        for(int j = 0;j <= N;j++){
            DP[i+1][j] += DP[i][j];
            for(int k = C;k*g <= j && k <= D;k++){
                DP[i+1][j-k*g] += DP[i][j] * combination.C(j,k*g) * combination.fact(k*g) * combination.rfact(k) * g_rfact.pow(k);
            }
        }
    }
    // cout << "!!" << endl;
