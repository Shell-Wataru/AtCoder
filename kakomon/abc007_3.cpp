...
int main()
{
    ...
    vector<vector<bool>> pushed(R,vector<bool>(C,false));
    queue<vector<ll>> q;
    q.push({0,sy,sx});
    ...
    while(!q.empty()){
        auto data = q.front();
        q.pop();
        D[data[1]][data[2]] = data[0];
        // cout << data[1] << "," << data[2] << endl;
        for(auto d:directions){
            ll next_x = data[1] + d.first;
            ll next_y = data[2] + d.second;
            if (0 <= next_x && next_x < R && 0 <= next_y  && next_y < C && !pushed[next_x][next_y] && G[next_x][next_y] == '.'){
                pushed[next_x][next_y] = true;
                q.push({data[0]+1,next_x,next_y});
            }
        }
    }
    cout << D[gy][gx] << endl;
