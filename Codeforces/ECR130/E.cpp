...
ll distance(vector<pair<ll, ll>> &P, ll i, ll j)
{
...
int solve()
{
  ...
  for (int i = 0; i < n; i++)
  {
    ...
    for (int j = 0; j < n; j++)
    {
      ...
      distances.push_back({distance(P, i, j), j});
    }
    sort(distances.begin(), distances.end());
    for (auto p : distances)
    {
      if (p.first == distances[0].first)
      {
        G[i].insert(p.second);
      }
      G[i].insert(i);
  ...
  for (int i = 0; i < n; i++)
  {
    ...
    if (all_same){
      if (i == *G[i].begin()){
        vector<modint> nextDP(n + 1, 0);
        for(int j = 1;j <= n;j++){
          nextDP[j] += DP[j-1];
        }
        for(int j = G[i].size();j <= n;j++){
          nextDP[j] += DP[j-G[i].size()];
        }
        DP = nextDP;
      }
    }else{
