...
int solve()
{
  ...
  SegmentTree<monoid_sum> seg_left(n);
  SegmentTree<monoid_sum> seg_right(n);
  ll ans = 0;
  for(int b = 1;b < n;b++){
    seg_left.update(P[b-1],1);
    for(int c = b+1;c < n;c++){
      seg_right.update(P[c],1);
    }
    for(int c = b+1;c < n;c++){
      seg_right.update(P[c],0);
      // cout << "b:" <<  b << "c:" << c << endl;
      // cout << seg_left.query(0,P[c]) << ","<< seg_right.query(0,P[b]) << endl;
      ans += seg_left.query(0,P[c]) * seg_right.query(0,P[b]);
    }
  }
  cout << ans << endl;
  return 0;
